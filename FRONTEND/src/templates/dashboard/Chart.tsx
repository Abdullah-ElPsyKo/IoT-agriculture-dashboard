...
const sortMonthlyAverages = (monthlyAverages: { time: string; amount: number }[]) => {
  return monthlyAverages.sort((a, b) => getMonthIndex(a.time) - getMonthIndex(b.time));
};
...
const Chart = () => {
  ...
  React.useEffect(() => {
    ...
    fetchAllCityData(selectedCity).then((weatherData: WeatherData[]) => {
  ...
  return (
    <React.Fragment>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <CustomSelect selectedCity={selectedCity} setSelectedCity={setSelectedCity} cities={cities} />
      </div>
      <LineChart
        dataset={chartData}
        margin={{ top: 16, right: 20, left: 70, bottom: 30 }}
        xAxis={[{ scaleType: "point", dataKey: "time", tickNumber: 12, tickLabelStyle: theme.typography.body2 as ChartsTextStyle }]}
        yAxis={[{
          label: "Temperature (°C)",
          labelStyle: { ...theme.typography.body1 as ChartsTextStyle, fill: theme.palette.text.primary },
          tickLabelStyle: theme.typography.body2 as ChartsTextStyle,
          max: 40,
          tickNumber: 3,
        }]}
        series={[{ dataKey: "amount", showMark: false, color: theme.palette.primary.light }]}
        sx={{
          [`.${axisClasses.root} line`]: { stroke: theme.palette.text.secondary },
          [`.${axisClasses.root} text`]: { fill: theme.palette.text.secondary },
          [`&.${axisClasses.left}.${axisClasses.label}`]: { transform: "translateX(-25px)" },
        }}
      />
    </React.Fragment>
  );
